...
void * worker_main(void * arg) {
	...
	while(forever) {
		//busywait for exactly 1 second
		get_elapsed_busywait(1, 0);
		clock_gettime(CLOCK_MONOTONIC, &timestamp);
		timestamp_in_dec = timestamp.tv_sec + (double)timestamp.tv_nsec / NANO_IN_SEC; 
		printf("[#WORKER#] %.9lf Still Alive!\n", timestamp_in_dec);
		sleep(1);
	}
	return NULL; 
}
...
void handle_connection(int conn_socket)
{
	...
	if (pthread_create(&worker, NULL, worker_main, (void*) &data) != 0) {
